...
const App = props => {
  ...
  useEffect(() => {
    ...
    for (let i = 0; i < 3; i++)
      soundPlayers.push(new Audio.Sound(soundList[i] + '.mp3', Audio.Sound.MAIN_BUNDLE));
      ...
      <ScrollView>
        <Center>
          ...
          <VStack space={5}>
            ...
            <TouchableOpacity onPress={() => showTimePicker()} style={{ width: 70 }}>
              <Text style={{ color: "gray" }}>Pick time</Text>
            ...
            <DateTimePicker
              ...
              mode="time"
              display='default'
              onConfirm={currDate => setCreateDate(currDate)}
